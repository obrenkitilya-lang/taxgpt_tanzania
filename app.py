...
class FSCase(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    business_name = db.Column(db.String(200), nullable=False)
    tin = db.Column(db.String(50), nullable=True)
    period = db.Column(db.String(100), nullable=False)
    service_type = db.Column(db.String(50), default='audit')
    status = db.Column(db.String(50), default='submitted')
    notes = db.Column(db.Text, nullable=True)
    admin_notes = db.Column(db.Text, nullable=True)
    tra_credentials = db.Column(db.Text, nullable=True)
    tra_submission_requested = db.Column(db.Boolean, default=False)
    tra_submitted = db.Column(db.Boolean, default=False)
    created_at = db.Column(db.DateTime, default=db.func.current_timestamp())
    updated_at = db.Column(db.DateTime, default=db.func.current_timestamp(), onupdate=db.func.current_timestamp())
    documents = db.relationship('FSDocument', backref='case', lazy=True, cascade='all, delete-orphan')
    messages = db.relationship('FSMessage', backref='case', lazy=True, cascade='all, delete-orphan')
    def to_dict(self):
        return {'id': self.id, 'business_name': self.business_name, 'tin': self.tin, 'period': self.period,
                'service_type': self.service_type, 'status': self.status, 'notes': self.notes,
                'tra_submission_requested': self.tra_submission_requested, 'tra_submitted': self.tra_submitted,
                'created_at': self.created_at.strftime('%Y-%m-%d %H:%M'), 'updated_at': self.updated_at.strftime('%Y-%m-%d %H:%M')}

class FSDocument(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    case_id = db.Column(db.Integer, db.ForeignKey('f_s_case.id'), nullable=False)
    filename = db.Column(db.String(255), nullable=False)
    file_path = db.Column(db.String(500), nullable=False)
    doc_type = db.Column(db.String(50), default='upload')
    uploaded_by = db.Column(db.String(20), default='client')
    uploaded_at = db.Column(db.DateTime, default=db.func.current_timestamp())
    def to_dict(self):
        return {'id': self.id, 'filename': self.filename, 'doc_type': self.doc_type,
                'uploaded_by': self.uploaded_by, 'uploaded_at': self.uploaded_at.strftime('%Y-%m-%d %H:%M')}

class FSMessage(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    case_id = db.Column(db.Integer, db.ForeignKey('f_s_case.id'), nullable=False)
    ...
    created_at = db.Column(db.DateTime, default=db.func.current_timestamp())
    def to_dict(self):
        return {'id': self.id, 'sender': self.sender, 'sender_email': self.sender_email,
                'content': self.content, 'is_request': self.is_request,
...
class DisputeCase(db.Model):
    ...
    notice_date = db.Column(db.String(50), nullable=True)
    ...
    created_at = db.Column(db.DateTime, default=db.func.current_timestamp())
    updated_at = db.Column(db.DateTime, default=db.func.current_timestamp(), onupdate=db.func.current_timestamp())
    documents = db.relationship('DisputeDocument', backref='case', lazy=True, cascade='all, delete-orphan')
    messages = db.relationship('DisputeMessage', backref='case', lazy=True, cascade='all, delete-orphan')
    def to_dict(self):
        return {'id': self.id, 'business_name': self.business_name, 'tin': self.tin,
                'dispute_type': self.dispute_type, 'notice_date': self.notice_date,
                'deadline_date': self.deadline_date, 'description': self.description,
                'status': self.status, 'created_at': self.created_at.strftime('%Y-%m-%d %H:%M'),
                'updated_at': self.updated_at.strftime('%Y-%m-%d %H:%M')}

class DisputeDocument(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    case_id = db.Column(db.Integer, db.ForeignKey('dispute_case.id'), nullable=False)
    filename = db.Column(db.String(255), nullable=False)
    file_path = db.Column(db.String(500), nullable=False)
    doc_type = db.Column(db.String(50), default='notice')
    uploaded_by = db.Column(db.String(20), default='client')
    uploaded_at = db.Column(db.DateTime, default=db.func.current_timestamp())
    def to_dict(self):
        return {'id': self.id, 'filename': self.filename, 'doc_type': self.doc_type,
...
@app.route("/api/fs/cases", methods=["POST"])
@login_required
def create_fs_case():
    try:
        business_name = request.form.get("business_name", "").strip()
        period = request.form.get("period", "").strip()
        if not business_name or not period:
            return jsonify({"error": "Business name and period are required"}), 400
        tra_requested = request.form.get("tra_submission_requested") == "true"
        case = FSCase(user_id=current_user.id, business_name=business_name,
                      tin=request.form.get("tin",""), period=period,
                      service_type=request.form.get("service_type","audit"),
                      notes=request.form.get("notes",""),
                      tra_submission_requested=tra_requested,
                      tra_credentials=request.form.get("tra_credentials","") if tra_requested else None)
        db.session.add(case)
        db.session.flush()
        for f in request.files.getlist("documents"):
            if f and f.filename:
                path, uname = save_uploaded_file(f, f"fs/{case.id}")
                db.session.add(FSDocument(case_id=case.id, filename=uname, file_path=path, doc_type="upload", uploaded_by="client"))
        db.session.commit()
        return jsonify({"message": "Case submitted successfully", "case_id": case.id})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/fs/cases", methods=["GET"])
@login_required
def get_fs_cases():
    if current_user.role in ['admin','tax_professional']:
        cases = FSCase.query.order_by(FSCase.created_at.desc()).all()
    else:
        cases = FSCase.query.filter_by(user_id=current_user.id).order_by(FSCase.created_at.desc()).all()
    result = []
    for c in cases:
        d = c.to_dict()
        d['document_count'] = len(c.documents)
        result.append(d)
    return jsonify(result)

@app.route("/api/fs/cases/<int:case_id>", methods=["GET"])
@login_required
def get_fs_case(case_id):
    case = FSCase.query.get_or_404(case_id)
    if case.user_id != current_user.id and current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    d = case.to_dict()
    d['documents'] = [doc.to_dict() for doc in case.documents]
    d['messages'] = [msg.to_dict() for msg in case.messages]
    return jsonify(d)

@app.route("/api/fs/cases/<int:case_id>/status", methods=["PUT"])
@login_required
def update_fs_status(case_id):
    if current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    case = FSCase.query.get_or_404(case_id)
    data = request.get_json()
    if 'status' in data: case.status = data['status']
    if 'tra_submitted' in data:
        case.tra_submitted = data['tra_submitted']
        if data['tra_submitted']: case.tra_credentials = None
    db.session.commit()
    return jsonify({"message": "Updated"})

@app.route("/api/fs/cases/<int:case_id>/documents", methods=["POST"])
@login_required
def upload_fs_document(case_id):
    case = FSCase.query.get_or_404(case_id)
    if case.user_id != current_user.id and current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    uploaded_by = 'admin' if current_user.role in ['admin','tax_professional'] else 'client'
    doc_type = request.form.get("doc_type", "output" if uploaded_by=="admin" else "upload")
    docs = []
    for f in request.files.getlist("documents"):
        if f and f.filename:
            path, uname = save_uploaded_file(f, f"fs/{case_id}")
            db.session.add(FSDocument(case_id=case_id, filename=uname, file_path=path, doc_type=doc_type, uploaded_by=uploaded_by))
            docs.append(uname)
    db.session.commit()
    return jsonify({"message": f"Uploaded {len(docs)} file(s)"})

@app.route("/api/fs/cases/<int:case_id>/documents/<int:doc_id>/download")
@login_required
def download_fs_document(case_id, doc_id):
    doc = FSDocument.query.get_or_404(doc_id)
    case = FSCase.query.get_or_404(case_id)
    if case.user_id != current_user.id and current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    return send_file(doc.file_path, as_attachment=True, download_name=doc.filename)

@app.route("/api/fs/cases/<int:case_id>/messages", methods=["POST"])
@login_required
def post_fs_message(case_id):
    case = FSCase.query.get_or_404(case_id)
    if case.user_id != current_user.id and current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    data = request.get_json()
    sender = 'admin' if current_user.role in ['admin','tax_professional'] else 'client'
    is_request = data.get('is_request', False) and sender == 'admin'
    db.session.add(FSMessage(case_id=case_id, sender=sender, sender_email=current_user.email,
                             content=data.get('content',''), is_request=is_request))
    if is_request: case.status = 'docs_requested'
    db.session.commit()
    return jsonify({"message": "Sent"})

# ── Dispute Routes ────────────────────────────────────────────────────────────

@app.route("/api/disputes/cases", methods=["POST"])
@login_required
def create_dispute_case():
    try:
        business_name = request.form.get("business_name","").strip()
        dispute_type = request.form.get("dispute_type","").strip()
        if not business_name or not dispute_type:
            return jsonify({"error": "Business name and dispute type are required"}), 400
        case = DisputeCase(user_id=current_user.id, business_name=business_name,
                           tin=request.form.get("tin",""), dispute_type=dispute_type,
        ...
        for f in request.files.getlist("documents"):
            if f and f.filename:
                path, uname = save_uploaded_file(f, f"disputes/{case.id}")
                db.session.add(DisputeDocument(case_id=case.id, filename=uname, file_path=path, doc_type="notice", uploaded_by="client"))
        db.session.commit()
        return jsonify({"message": "Dispute case submitted successfully", "case_id": case.id})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/disputes/cases", methods=["GET"])
@login_required
def get_dispute_cases():
    if current_user.role in ['admin','tax_professional']:
        cases = DisputeCase.query.order_by(DisputeCase.created_at.desc()).all()
    else:
        cases = DisputeCase.query.filter_by(user_id=current_user.id).order_by(DisputeCase.created_at.desc()).all()
    result = []
    for c in cases:
        d = c.to_dict()
        d['document_count'] = len(c.documents)
...
@app.route("/api/disputes/cases/<int:case_id>", methods=["GET"])
@login_required
def get_dispute_case(case_id):
    case = DisputeCase.query.get_or_404(case_id)
    if case.user_id != current_user.id and current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    d = case.to_dict()
    d['documents'] = [doc.to_dict() for doc in case.documents]
...
@app.route("/api/disputes/cases/<int:case_id>/status", methods=["PUT"])
@login_required
def update_dispute_status(case_id):
    if current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    case = DisputeCase.query.get_or_404(case_id)
    data = request.get_json()
...
@app.route("/api/disputes/cases/<int:case_id>/documents", methods=["POST"])
@login_required
def upload_dispute_document(case_id):
    case = DisputeCase.query.get_or_404(case_id)
    if case.user_id != current_user.id and current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    uploaded_by = 'admin' if current_user.role in ['admin','tax_professional'] else 'client'
    doc_type = request.form.get("doc_type", "response" if uploaded_by=="admin" else "supporting")
    docs = []
    for f in request.files.getlist("documents"):
        if f and f.filename:
            path, uname = save_uploaded_file(f, f"disputes/{case_id}")
            db.session.add(DisputeDocument(case_id=case_id, filename=uname, file_path=path, doc_type=doc_type, uploaded_by=uploaded_by))
            docs.append(uname)
    db.session.commit()
    return jsonify({"message": f"Uploaded {len(docs)} file(s)"})

@app.route("/api/disputes/cases/<int:case_id>/documents/<int:doc_id>/download")
@login_required
def download_dispute_document(case_id, doc_id):
    doc = DisputeDocument.query.get_or_404(doc_id)
    case = DisputeCase.query.get_or_404(case_id)
    if case.user_id != current_user.id and current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    return send_file(doc.file_path, as_attachment=True, download_name=doc.filename)

@app.route("/api/disputes/cases/<int:case_id>/messages", methods=["POST"])
@login_required
def post_dispute_message(case_id):
    case = DisputeCase.query.get_or_404(case_id)
    if case.user_id != current_user.id and current_user.role not in ['admin','tax_professional']:
        return jsonify({"error": "Access denied"}), 403
    data = request.get_json()
    sender = 'admin' if current_user.role in ['admin','tax_professional'] else 'client'
    is_request = data.get('is_request', False) and sender == 'admin'
    db.session.add(DisputeMessage(case_id=case_id, sender=sender, sender_email=current_user.email,
                                   content=data.get('content',''), is_request=is_request))
    if is_request: case.status = 'docs_requested'
    db.session.commit()
    return jsonify({"message": "Sent"})
